...
## ADR-43 · El Cap. XIV, Fase 1: el periodo se calcula, la ruta se hereda, el auditor se verifica — 2026-09-08

**Contexto.** El Capítulo XIV es el último bloque grande del calendario (primer periodo: todo 2028, Transitorio Octavo). Pide una revisión anual dictaminada, y reparte quién puede firmarla según el riesgo del propio obligado. Se construye en tres fases; esta es la primera: **cuándo** corre el periodo, **quién** puede dictaminarlo y **cuándo** vence la entrega. El dictamen en sí —los cinco resultados del Art. 48, los hallazgos del 49— es otra fase, y nada aquí toca contenido de auditoría.

**Decisión.** Cinco piezas:

1. **La ruta no se recalcula: se hereda del ADR-28.** «Riesgo alto ⇒ auditor externo» ya lo derivaba la evaluación de entidad desde el 29-ago. Escribirla otra vez aquí habría hecho tres copias de una regla regulatoria, así que se extrajo a una función única (`auditoriaQueCorresponde`) y las tres la llaman. Hoy coincidían; el día que una cambiara, las otras seguirían calculando en silencio.
2. **La ruta se congela con su evaluación, y envejece.** El periodo guarda contra qué evaluación se decidió. Si después el grado cambia, el periodo abierto no se reescribe: se dice sobre qué se fijó y si hoy daría otra cosa — el criterio del ADR-25 por cuarta vez.
3. **El Art. 44 no se registra: se verifica.** Que el auditor interno no sea el REC y que tenga al menos una constancia de capacitación son hechos que VIZO comprueba contra la plantilla y las asistencias del Cap. XII. Es la primera vez que un capítulo usa otro como requisito verificable, y convierte la capacitación de seguimiento en prueba.
4. **Hizo falta una fecha nueva: `inicio_de_operaciones`.** No sirve `fecha_alta_autoridad` —el alta ante el SAT—: se puede operar antes de darse de alta o al revés, y el Art. 43 cuelga del inicio de operaciones. Es la lección del ADR-34 otra vez: dos hechos que se parecen no son el mismo hecho. En NULL, el periodo ordinario se calcula pero se dice que no se sabe si tocaba el largo.
5. **El último día hábil de marzo se dice a medias, a propósito.** Los fines de semana son ciertos sin catálogo; los feriados oficiales no están en el Acuerdo y el calendario de 2029 no existe todavía. `dias_inhabiles` nace vacía y la fecha viaja con `conCalendario: false`, porque sin calendario la fecha calculada puede caer **después** de la real — la dirección arriesgada, y por eso se declara en vez de presumirse.

**Los tropiezos del camino, y son míos:** cité el Transitorio Octavo en la línea 749, que es el Séptimo. El periodo del Art. 43 dejaba abrir además un periodo ordinario de los años que ese periodo largo ya cubre —dos periodos solapados, ambos plausibles, uno falso: el modo de falla de la regla dura 6 exactamente—. Y la regla del Art. 45, la más consecuente del capítulo, quedó primero solo como precondición de TypeScript, cuando CLAUDE.md dice que la precondición es el último recurso; hoy también es trigger.

**Fijado con:** las 17 aserciones de la migración `20260908120000` (patrón GUARD), pruebas de dominio y de persistencia, y sabotajes confirmados en rojo y restaurados —incluido tirar el trigger del Art. 45 y ver morir la migración—. Las seis guardas estructurales en cero.
...
18. **¿Qué periodo audita quien empieza a operar justo en 2028?** *(8-sep-2026, ADR-43)* — Dos normas apuntan a periodos distintos y ninguna cede expresamente. El **Transitorio Octavo** (línea 750) dice que «el primer periodo de revisión de auditoría iniciará el primero de enero de dos mil veintiocho y concluirá el treinta y uno de diciembre del mismo año», pero lo acota literalmente «para efectos del artículo 42». El **Art. 43** (línea 458) da al primer año de operaciones un periodo largo, «desde la fecha en que inicien operaciones […] y hasta el treinta y uno de diciembre del siguiente año». Para quien ya operaba antes de 2028 no hay conflicto: le toca 2028 completo. Para quien **inicia operaciones dentro de 2028**, el Octavo diría 2028 y el Art. 43 diría inicio→31-dic-2029. VIZO implementa que **gana el Art. 43**, por ser la regla especial y porque el propio Octavo se limita al 42 — pero es lectura nuestra, no del texto. El costo de equivocarse es un periodo auditado mal delimitado, y por tanto un dictamen que cubre meses que no eran. Es cambio de código en `periodoDeAuditoria` (`src/dominio/auditoria.ts`), no un dato de catálogo: la forma de la regla, como la 12.

19. **¿De dónde sale el calendario de días inhábiles del Art. 50?** *(8-sep-2026, ADR-43)* — El Art. 50 ¶1 fija la entrega del dictamen «a más tardar el último día hábil del mes de marzo» y el Acuerdo no dice qué días son inhábiles. ¿Rige el calendario de días inhábiles que publica el SAT, el del Art. 74 de la LFT, o el que fije el propio obligado en su Manual? La tabla `dias_inhabiles` nace vacía y con `fuente` obligatoria por fila justo para que la respuesta sea un `INSERT` con su cita. Mientras tanto VIZO retrocede solo fines de semana y **declara** que la fecha puede caer después de la real. Es la pregunta barata de la lista: no bloquea nada y se resuelve cargando un año.
