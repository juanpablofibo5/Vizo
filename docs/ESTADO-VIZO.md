...
pieza. **Fecha de corte: 30-ago-2026.**
...
| 18 | **Evaluación de entidad** (riesgo del obligado → tipo de auditoría, Arts. 44/45) | ✅ **Esquema, motor, persistencia y pantalla construidos (29-ago, ADR-28):** escala de efectividad ordinal con evidencia exigible por nivel, residual por elemento con tope estructural, histórico append-only con la consecuencia de los Arts. 44/45 ya resuelta (`externa_obligatoria`/`interna_permitida`). **Pantalla `/entidad` construida (30-ago):** el veredicto de auditoría se lee antes que el número, el inherente y el residual salen siempre juntos, y sin configuración muestra el hueco sin ofrecer evaluar. La sesión con Luis valida contra lo construido | ADR-28 · migración `20260829150000` · `/entidad` · `tests/persistencia/entidad.test.ts` |
| 19 | **MER como documento exportable** | ✅ **Construido (29-ago, ADR-29):** se EMITE, no se descarga — texto congelado con SHA-256, solo del modelo vigente, ocho secciones con respaldo por afirmación y pendientes con artículo, la consecuencia de los Arts. 44/45 escrita, y dedupe por contenido. **Pantalla `/mer` construida (30-ago):** emisiones con su huella, y sin modelo vigente lo dice en vez de ofrecer el botón. La celda vacía del mercado (BMK-01: ningún competidor lo declara) ahora tiene producto | ADR-29 · migración `20260829200000` · `/mer` · `tests/persistencia/mer.test.ts` |
...
| 21 | Screening de listas (OFAC · ONU · LPB · 69-B · PEP) | ◐ **Conector construido (29-ago, ADR-30):** listas versionadas con hash, matching por trigramas + RFC exacto, consulta con snapshot que nace pendiente, resolución humana única con razonamiento, alerta que nombra su consulta. **Pantalla construida (30-ago)** en el expediente y ANTES de la puerta del ADR-24 —consultar listas precede a decidir si se abre relación—, con la resolución humana y su razonamiento; el operador de pg_trgm quedó calificado con su esquema tras un defecto que solo aparecía con el rol de la aplicación. **Pendiente:** parsers ONU/LPB (vía provisional en runbook 06), alias de OFAC, y la **primera carga real de las cuatro listas** — sin ella la consulta se detiene a propósito. PEP sigue como dependencia comercial aparte, no se vende | ADR-30 · migración `20260829230000` · runbook 06 · expediente → Listas de control · `tests/.../screening.test.ts` |
