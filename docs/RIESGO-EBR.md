...
- ~~**Tabla `perfil_transaccional`, que no existe.**~~ ✅ **Construida el 21-ago-2026** como `perfiles_transaccionales` (ADR-22). Salió como se anticipó —histórico append-only, no columna—, con dos cosas que este contraste no había separado: el ancla del reloj es **la fecha del acto**, no la de captura ni la de vigencia; y los tres «seis meses» del Art. 23 Ter 1 son **dos** plazos distintos, no uno (¶2 y ¶3 segunda oración son la misma frontera; ¶3 primera oración es la cadencia). Ambos viven en `parametros_motor` con fuente propia.

- ~~**El enum `tipo_alerta` no tiene categoría para desviación de perfil.**~~ ✅ **Cerrado el 21-ago-2026.** Se agregaron **dos** valores, no uno: `desviacion_perfil` y `perfil_ausente`. La separación no fue previsión sino un CHECK que se negó a aceptar una alerta de desviación sin perfil — «el cliente operó sin declarar» no es una desviación, no hay perfil del cual desviarse, y meterlos en el mismo valor habría obligado a volver `perfil_id` opcional para ambos, perdiendo la única garantía de que toda desviación pueda decir contra qué se desvió. Y `alertas` ganó `perfil_id` y `operacion_id`, tal como este contraste anticipó que la FK a `evaluaciones_umbral` no le quedaba.
